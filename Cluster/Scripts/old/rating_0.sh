...
vectorize () {
	python3 $1/vectorize.py $2 $3 $4 $5 $6
	#echo $1 $2 $3 $4
}
...
usage() { 
	echo "Usage: $0 [-m <msa input path>] [-o <output dir>] [-w <window size>] [-s <step size] [-i inputcluster] [-c codontable] [-p ]" 1>&2; exit 1; 
}

while getopts ":m:o:w:s:i:c:p:" option ; do
	case ${option} in
	m) inmsa=${OPTARG} ;;
	o) outdir=${OPTARG} ;;
	w) win=${OPTARG} ;;
...
if ! [ -f "$inmsa" ] || ! [ -f "$inclstr" ] || ! [ -f "$incodon" ] || [ -z "$win" ] || [ -z "$stp" ] || [ -z "$proc" ] || ! [ -d "$outdir" ]; then
	usage
fi
...
ls ${outdir}cluster/ | parallel --progress --keep-order --jobs $proc "vectorize $scriptdir ${outdir}vectors $incodon $stp $win ${outdir}cluster/{}"
